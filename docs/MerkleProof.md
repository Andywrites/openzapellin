...
View Source: [contracts/cryptography/MerkleProof.sol](../contracts/cryptography/MerkleProof.sol)
...
- [verify(bytes32[] proof, bytes32 root, bytes32 leaf)](#verify)
...
```js
function verify(bytes32[] proof, bytes32 root, bytes32 leaf) internal
returns(bool)
```

...
and a public setter for metadata URI (ERC721FullMock.sol)

View Source: [contracts/mocks/ERC721FullMock.sol](../contracts/mocks/ERC721FullMock.sol)
...
- [setTokenURI(uint256 tokenId, string uri)](#settokenuri)
- [removeTokenFrom(address from, uint256 tokenId)](#removetokenfrom)
...
```js
function setTokenURI(uint256 tokenId, string uri) public undefined
```
...
```js
function removeTokenFrom(address from, uint256 tokenId) public undefined
```

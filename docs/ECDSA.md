...
View Source: [contracts/cryptography/ECDSA.sol](../contracts/cryptography/ECDSA.sol)
...
```js
function recover(bytes32 hash, bytes signature) internal
returns(address)
```
...
```js
function toEthSignedMessageHash(bytes32 hash) internal
returns(bytes32)
```

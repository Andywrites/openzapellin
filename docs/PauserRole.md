...
View Source: [contracts/access/roles/PauserRole.sol](../contracts/access/roles/PauserRole.sol)

**↘ Derived Contracts: [Pausable](Pausable.md), [PauserRoleMock](PauserRoleMock.md)**
...
```js
struct Roles.Role private pausers;
...
```js
function isPauser(address account) public
returns(bool)
```
...
```js
function addPauser(address account) public undefined onlyPauser 
```
...
```js
function renouncePauser() public undefined
```
...
```js
function _addPauser(address account) internal undefined
```
...
⤿ Overridden Implementation(s): [PauserRoleMock._removePauser](PauserRoleMock.md#_removepauser)

```js
function _removePauser(address account) internal undefined
```

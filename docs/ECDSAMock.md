...
View Source: [contracts/mocks/ECDSAMock.sol](../contracts/mocks/ECDSAMock.sol)
...
```js
function recover(bytes32 hash, bytes signature) public
returns(address)
```
...
```js
function toEthSignedMessageHash(bytes32 hash) public
returns(bytes32)
```
